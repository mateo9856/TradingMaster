...
Keep `.env`, exchange API keys, database passwords, Kafka credentials, and `AUTH_SECRET` local — public candle streams don't need exchange keys. Key env vars: `TIMESCALE_URL`, `KAFKA_BOOTSTRAP_SERVERS`, `PRODUCER_CONFIG_REFRESH_SECONDS`, `FX_REFERENCE_EXCHANGE`/`FX_MAX_AGE_SECONDS`, `PROMETHEUS_METRICS_ENABLED`, `OTEL_ENABLED`/`OTEL_EXPORTER_OTLP_ENDPOINT`, `LOG_LEVEL`, `AUTH_SECRET`/`AUTH_TOKEN_LIFETIME_SECONDS` (JWT signing for FastAPI Users — the checked-in default is dev-only, override it everywhere else). Markets are not env config — they live in the `exchanges`/`symbols` tables. The Flink job additionally needs `TIMESCALE_JDBC_URL`, `TIMESCALE_USER`, `TIMESCALE_PASS` (optional: `FLINK_CHECKPOINT_INTERVAL`, `FLINK_TOPIC_DISCOVERY_INTERVAL`, `FLINK_CHECKPOINT_DIR`) exported directly in its shell (activating `env_flink` does not load `.env`). Never include secrets or generated database files in patches.
